...
import { Plus, Edit2, Trash2, Users, MapPin, FileText, Phone, Mail, MessageCircle, Camera, X } from 'lucide-react';
...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ onNavigate }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!editingId && !formData.email) {
      alert('Please enter email for new lawyer');
      return;
    }

    if (!editingId && !formData.lawyerPassword) {
      alert('Please enter password for new lawyer');
      return;
    }
    ...
    if (editingId) {
      ...
      updatedLawyers = lawyers.map((l) =>
        l.id === editingId ? { ...l, ...lawyerData } as LawyerProfile : l
      );
      setEditingId(null);
    } else {
      // Add new lawyer
      const newLawyer: LawyerProfile = {
        ...
        ...lawyerData as LawyerProfile,
      };
      updatedLawyers = [...lawyers, newLawyer];

      // Save lawyer credentials to localStorage
      const existingCredentials = localStorage.getItem('lawyerCredentials');
      const credentials: LawyerCredentials[] = existingCredentials ? JSON.parse(existingCredentials) : [];
      credentials.push({
        id: newLawyer.id,
        name: newLawyer.name,
          ...
          <div className="bg-white rounded-xl shadow-lg p-8 mb-8 border border-slate-200">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Lawyer Password (only for new lawyers) */}
                {!editingId && (
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 mb-2">
                      Login Password *
                    ...
                    <input
                      type="password"
                      name="lawyerPassword"
                      value={formData.lawyerPassword || ''}
                      onChange={handleInputChange}
                      placeholder="Set password for lawyer login"
                      className="w-full px-4 py-2 border border-slate-300 rounded-lg focus:outline-none focus:border-blue-600"
                      required
                    />
                  </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-4">
                  {formData.photo ? (
                    ...
                    <div className="w-24 h-24 rounded-full bg-slate-200 flex items-center justify-center border-2 border-dashed border-slate-300">
                  ...
                  <div className="flex-1">
                    ...
                    <label
                      ...
                      className="inline-flex items-center gap-2 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg cursor-pointer transition-all"
                    >
