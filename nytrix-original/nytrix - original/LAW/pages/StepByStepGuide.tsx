...
import { ChevronRight, AlertCircle, Phone, Heart } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950 py-8">
      <div className="max-w-5xl mx-auto px-4">
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-4xl font-bold bg-gradient-to-r from-emerald-300 via-teal-300 to-cyan-300 bg-clip-text text-transparent mb-2">📋 Step-by-Step Action Guide</h1>
          <p className="text-emerald-200 text-lg">Detailed guidance for navigating legal situations</p>
        </div>
            ...
            <button
              ...
              className={`p-4 rounded-lg border-2 text-left transition-all ${
                selectedGuide === guide.id
                  ? 'border-emerald-400 bg-gradient-to-br from-emerald-500/30 to-teal-500/30 shadow-lg shadow-emerald-500/30'
                  : 'border-emerald-400/30 bg-purple-800/40 hover:border-emerald-400'
              }`}
            >
              <div className="text-2xl mb-2">{guide.icon}</div>
              <p className="font-bold text-emerald-100 text-sm">{guide.label}</p>
            </button>
          ))}
        </div>

        {/* Warning Banner */}
        <div className="bg-red-500/20 border-l-4 border-red-400 p-6 rounded-r-lg mb-8 backdrop-blur">
        ...
        <div className="space-y-4">
          {steps.map((step) => {
            ...
            return (
              <div key={step.step} className="bg-gradient-to-br from-purple-800/40 to-purple-900/40 border border-emerald-400/30 rounded-lg overflow-hidden backdrop-blur">
                <button
                  onClick={() => setExpandedStep(isExpanded ? null : step.step)}
                  className="w-full flex items-center justify-between p-5 hover:bg-purple-800/50 transition-colors"
                >
                  <div className="flex items-start gap-4 text-left flex-1">
                    ...
                    <div>
                      <h3 className="text-lg font-bold text-emerald-100">{step.title}</h3>
                      <p className="text-sm text-emerald-300 mt-1">{step.description}</p>
                      <p className="text-xs text-emerald-400 mt-2 font-semibold">⏱️ Timeline: {step.timeline}</p>
                    </div>
                  </div>
                  <ChevronRight className={`w-5 h-5 text-emerald-400 transition-transform flex-shrink-0 ${isExpanded ? 'rotate-90' : ''}`} />
                </button>

                {isExpanded && (
                  <div className="px-5 pb-5 pt-0 border-t border-emerald-400/30 space-y-4">
                    {/* Actions */}
                    <div>
                      <h4 className="font-bold text-emerald-100 mb-3">Action Items:</h4>
                    ...
                    {step.cautions && step.cautions.length > 0 && (
                      <div className="bg-amber-500/20 border border-amber-400/30 p-4 rounded-lg backdrop-blur">
                    ...
                    {step.relatedLaws && step.relatedLaws.length > 0 && (
                      <div>
                        <h4 className="font-bold text-emerald-100 mb-2">Related Laws:</h4>
                        <div className="flex flex-wrap gap-2">
                          {step.relatedLaws.map((law, idx) => (
                            <span
                              key={idx}
                              className="bg-purple-900/40 text-emerald-300 px-3 py-1 rounded-full text-xs font-semibold border border-emerald-400/30"
                            >
                              {law}
                            </span>
                          ))}
        ...
        <div className="mt-8 bg-gradient-to-br from-emerald-500/20 to-teal-500/20 border border-emerald-400/30 p-6 rounded-lg backdrop-blur">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="bg-purple-900/30 p-4 rounded-lg border border-emerald-400/20">
              <p className="font-bold text-emerald-200 mb-2">Crisis Helplines:</p>
            ...
            <div className="bg-purple-900/30 p-4 rounded-lg border border-emerald-400/20">
              <p className="font-bold text-emerald-200 mb-2">Self-Care Tips:</p>
              <p className="text-emerald-300 text-sm">
        ...
        {/* Call to Action */}
        <div className="mt-8 text-center bg-gradient-to-br from-slate-950 via-purple-950 to-slate-950 text-white p-8 rounded-lg border border-emerald-400/30">
          <h3 className="text-2xl font-bold mb-3 text-emerald-100">Need Professional Help?</h3>
          <p className="mb-4 opacity-90 text-emerald-200">
          ...
          <button 
            ...
            className="bg-gradient-to-r from-emerald-500 to-teal-600 text-white px-6 py-3 rounded-lg font-bold hover:shadow-lg hover:shadow-emerald-500/30 transition-all inline-flex items-center gap-2"
          >
